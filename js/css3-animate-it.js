...
(function(jQuery) {
  ...
  jQuery.expr[":"]["appeared"] = function(element) {
    ...
    if (
      top + jQueryelement.height() >= window_top &&
      top - (jQueryelement.data("appear-top-offset") || 0) <=
        window_top + jQuerywindow.height() &&
      left + jQueryelement.width() >= window_left &&
      left - (jQueryelement.data("appear-left-offset") || 0) <=
        window_left + jQuerywindow.width()
    ) {
      return true;
    } else {
      return false;
...
jQuery(document.body).on("appear", ".animatedParent", function(
  e,
  jQueryaffected
) {
...
jQuery(document.body).on("disappear", ".animatedParent", function(
  e,
  jQueryaffected
) {
